...
func _resources_and_movement() -> void:
	...
	model = duel(586, 612)
	advance(model, 90, {"x": 1}, {"x": 1})
	check(model.fighters[1].x <= 612 and model.fighters[0].x <= 586, "right corner conserves push separation")
	model = duel(260, 302)
...
func _trades_and_rounds() -> void:
	...
	check(model.fighters[0].move == null and model.fighters[0].buffer_left == 0 and model.fighters[0].x == 206, "rematch clears movement and pending attacks")
