...
func _back_or_pause() -> void:
	...
	elif screen in ["setup", "help", "result"]:
		show_title()
...
func _validate_setup() -> void:
	...
	device_notice.text = "两位玩家请选择不同的操作设备" if duplicate else "60 秒 / 回合 · 先赢两局获胜\n手柄  X 轻攻 · Y 重攻 · A 技能 · B 投技"
...
func _devices_ready() -> bool:
	return router.connected(devices[0]) and (mode == "cpu" or router.connected(devices[1]))
