...
func reset(fighters: Array) -> void:
	center_x = clampf((fighters[0].x + fighters[1].x) * 0.5, Arena.HALF_VIEW, Arena.WIDTH - Arena.HALF_VIEW)
	zoom = Arena.ZOOM
	shake = Vector2.ZERO
...
func update(fighters: Array, delta: float, _artwork: Array[Rect2] = []) -> void:
	...
	var target := clampf((left + right) * 0.5, Arena.HALF_VIEW, Arena.WIDTH - Arena.HALF_VIEW)
	...
	var minimum := maxf(Arena.HALF_VIEW, right - Arena.HALF_VIEW + Arena.BODY_MARGIN)
	var maximum := minf(Arena.WIDTH - Arena.HALF_VIEW, left + Arena.HALF_VIEW - Arena.BODY_MARGIN)
