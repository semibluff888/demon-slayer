...
func consume(events: Array) -> void:
	for event: Dictionary in events:
		if event.type == "swing" and event.move in ["water_slash", "water_wheel", "iai", "thunder"]:
			var slot: int = event.attacker
			callouts[slot] = combat.moves[event.move].display_name
			callout_time[slot] = 1.15
...
func _draw() -> void:
	...
	for i in range(2):
		...
		_text("P1" if i == 0 else ("CPU" if cpu else "P2"), Vector2(x + 365, 43), 14, accent)
		...
		if callout_time[i] > 0:
			...
			var at := Vector2(45 if i == 0 else 931, 157)
			draw_rect(Rect2(at - Vector2(13, 24), Vector2(312, 39)), Color(0.03, 0.05, 0.1, alpha * 0.73))
			draw_line(at + Vector2(0, 16), at + Vector2(269, 16), Color(accent, alpha * 0.7), 1)
			_text(callouts[i], at + Vector2(0, 3), 23, Color(PAPER, alpha), true)
	...
	var seconds := str(ceili(combat.remaining / 60.0))
	...
	_text("ROUND %02d" % (combat.wins[0] + combat.wins[1] + 1), Vector2(605, 126), 11, GOLD)
	...
	_text("CPU" if cpu else "P2  " + input_hints[1], Vector2(867, 703), 13, MUTED)
